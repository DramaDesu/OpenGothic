...
Marvin::Marvin() {
  ...
  cmd = std::vector<Cmd>{
    ...
    {"synth backend %s",           C_SetSynthBackend},
    {"music provider %s",          C_SetSynthBackend},
    {"music list",                 C_MusicList},
    {"music solo %d",              C_MusicSolo},
    {"music mute %d",              C_MusicMute},
    {"music clear",                C_MusicClear},
    {"music pause",                C_MusicPause},
...
bool Marvin::exec(std::string_view v) {
  ...
  switch(ret.cmd.type) {
    ...
    case C_SetSynthBackend: {
      // "music provider dx|opengothic|gothickit" — switches the whole music provider
      const auto& arg = ret.argv[0];
      int providerIdx = -1;
      if(arg == "dx" || arg == "dxsynth" || arg == "dxmusic") {
        providerIdx = 0; // PROVIDER_DXMUSIC
        print("Music provider: DXMusicProvider (NT5 dmsynth)");
        } else if(arg == "opengothic" || arg == "og" || arg == "tsf") {
        providerIdx = 1; // PROVIDER_OPENGOTHIC
        print("Music provider: OpenGothicMusicProvider (TinySoundFont)");
        } else if(arg == "gothickit" || arg == "gk") {
        providerIdx = 2; // PROVIDER_GOTHICKIT
        print("Music provider: GothicKitMusicProvider");
        } else {
        print("Usage: music provider [dx|opengothic|gothickit]");
        return true;
        }
      Gothic::settingsSetI("INTERNAL", "soundProviderIndex", providerIdx);
      return true;
      }

    case C_MusicList: {
      std::vector<GameMusic::ChannelInfo> ch;
      GameMusic::inst().getChannelStates(ch);
      if(ch.empty()) {
        print("No active music channels");
        return true;
        }
      for(auto& c : ch) {
        char buf[256];
        std::snprintf(buf, sizeof(buf),
          "[%2zu] ch:%2u  patch:%06X  vol:%.2f  pan:%.2f  v:%zu%s%s  %s",
          c.index, c.pChannel, c.dwPatch,
          c.volume, c.pan, c.activeVoices,
          c.isMuted ? " [MUTED]" : "",
          c.isSolo  ? " [SOLO]"  : "",
          c.name.c_str());
        print(buf);
    ...
    case C_MusicSolo: {
      size_t idx = 0;
      if(!fromString(ret.argv[0], idx))
        return false;
      GameMusic::inst().setChannelSolo(idx);
      char buf[64];
      std::snprintf(buf, sizeof(buf), "Solo: channel %zu (type 'music clear' to reset)", idx);
      print(buf);
      return true;
      }
    ...
    case C_MusicMute: {
      ...
      if(!fromString(ret.argv[0], idx))
        return false;
      GameMusic::inst().setChannelMuted(idx, true);
      char buf[64];
      std::snprintf(buf, sizeof(buf), "Muted: channel %zu", idx);
      print(buf);
      return true;
      }

    case C_MusicClear: {
      GameMusic::inst().clearChannelIsolation();
      print("Channel isolation cleared");
      return true;
      }

    case C_MusicPause: {
      const bool nowEnabled = !GameMusic::inst().isEnabled();
      GameMusic::inst().setEnabled(nowEnabled);
      print(nowEnabled ? "Music resumed" : "Music paused");
      return true;
      }
    }
